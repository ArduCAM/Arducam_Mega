...
int PIN_CAM_RESETB      = -1;
int PIN_CAM_XCLK        = 3;
int PIN_CAM_VSYNC       = 2; // GP15 hsync  GP14 pixel clock
...
void arducam_capture_frame(struct arducam_config* config, uint8_t* image)
{
    ...
    dma_channel_configure(config->dma_channel, &c, config->image_buf, &config->pio->rxf[config->pio_sm],
                          config->image_buf_size, false);
