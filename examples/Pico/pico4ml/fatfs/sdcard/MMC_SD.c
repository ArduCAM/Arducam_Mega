...
static int sd_cmd(const cmdSupported cmd, uint32_t arg, bool isAcmd,
                  uint32_t *resp) {
...
static uint64_t sd_sectors_nolock() {
    ...
    case 0:
        ...
        mult =
            1 << (c_size_mult + 2); // MULT = 2^C_SIZE_MULT+2 (C_SIZE_MULT < 8)
...
static int in_sd_read_blocks(uint8_t *buffer, uint64_t ulSectorNumber,
...
int sd_read_blocks(uint8_t *buffer, uint64_t ulSectorNumber,
                   uint32_t ulSectorCount) {
    sd_spi_select();
...
static uint8_t sd_write_block(const uint8_t *buffer, uint8_t token,
                              uint32_t length) {
    uint16_t crc = (~0);
 ...
 */
static int in_sd_write_blocks(const uint8_t *buffer, uint64_t ulSectorNumber,
                              uint32_t blockCnt) {
    if (ulSectorNumber + blockCnt > sectors)
        return SD_BLOCK_DEVICE_ERROR_PARAMETER;
    if (m_Status & (STA_NOINIT | STA_NODISK))
    ...
    if (blockCnt == 1) {
        ...
        if (SD_BLOCK_DEVICE_ERROR_NONE !=
            (status = sd_cmd(CMD24_WRITE_BLOCK, addr, false, 0))) {
            return status;
        }
        // Write data
        ...
        if (SD_BLOCK_DEVICE_ERROR_NONE !=
            (status = sd_cmd(CMD25_WRITE_MULTIPLE_BLOCK, addr, false, 0))) {
            return status;
        }
        // Write the data: one block at a time
        do {
            response =
                sd_write_block(buffer, SPI_START_BLK_MUL_WRITE, _block_size);
            if (response != SPI_DATA_ACCEPTED) {
...
int sd_write_blocks(const uint8_t *buffer, uint64_t ulSectorNumber,
                    uint32_t blockCnt) {
    sd_spi_select();
...
static int sd_init_medium() {
    ...
    */
    ...
    if (SD_BLOCK_DEVICE_ERROR_NONE !=
        (status = sd_cmd(CMD58_READ_OCR, 0x0, false, &response))) {
        return status;
    }
