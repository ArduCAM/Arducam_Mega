...
char *htmls[] = {
    ...
    "'http://192.168.0.246:8080/'+cmd,true);httpRequest.send();httpRequest.onreadystatechange=function(){if(httpRequest.readyState==4&&httpRequest.status==200){var "
    "json=httpRequest.responseText;console.log(uint8Array(stringToUint8Array(json)));base64_data=window.btoa(stringToUint8Array(json).reduce((data, byte) => data + String.fromCharCode(byte), ''));console.log(base64_data);document."
    "getElementById('pic').src='data:image/png;base64,'+base64_data;}};} function stringToUint8Array(str){var arr = [];for (var i=0,j=str.length;i<j;++i){arr.push(str.charCodeAt(i));}var tmpUint8Array = new Uint8Array(arr);return tmpUint8Array;}function uint8Array(uint8Array) {  return Array.prototype.map .call(uint8Array, (x) => ('00' + x.toString(16)).slice(-2)) .join(''); }</script></html>",
    "HTTP/1.1 200 OK\r\nServer: jdbhttpd/0.1.0\r\nContent-Type: "
    "image/jpeg\r\n\r\n",
};
char buff_string[80];
...
void send_at_cmd(const char cmd[]) { printf("AT+%s\r\n", cmd); }
...
char *at_cmd[] = {"RESTORE",                        // 0
                  "CWMODE=1",                       // 1
                  "CWJAP=\"Arducam\",\"20160224\"", // 2
                  "CIPMUX=1",                       // 3
                  "CIPSERVER=1,8080",               // 4
                  "CIPSTA?",                        // 5
                  "CIPSEND",                        // 6
                  "CIPCLOSE"};                      // 7
uint8_t frame_buff[320*240*2];
...
int main() {
    ...
    struct ClassCamera Ardcam;
    Ardcam.cam = createArducamCamera(13);
    Ardcam.status = Camera_init;
    // };
        ...
        if (send_flag) {
            ...
            if (esp_cmd_idk < 6) {
                ...
                if (send_index == 0) {
                    ...
                    if (Ardcam.status == Camera_open) {
                        takePicture(&Ardcam.cam, CAM_IMAGE_MODE_QVGA,
                                    CAM_IMAGE_PIX_FMT_JPG);
                        len =
                            Ardcam.cam.totalLength + strlen(htmls[send_index]);
                    } else {
                        len = 0;
                        esp_cmd_idk = 7;
                        send_index = -1;
                ...
                if(len>=8192 ){
                    sprintf(buff_string, "%sL=%s,%d", at_cmd[esp_cmd_idk], client_id,
                                    len);
                }else{
                sprintf(buff_string, "%s=%s,%d", at_cmd[esp_cmd_idk], client_id,
                                    len);
                }
            
                send_at_cmd(buff_string);
                // debug_printf("****************%d-%d******", client_id[0],
                // client_id_cnt);
                ...
                case 1:
                    printf(htmls[send_index]);
                    if (Ardcam.status == Camera_open) {
                        int rs = readBuff(&Ardcam.cam, frame_buff,
                                          Ardcam.cam.totalLength);

                        uart_write_blocking(uart0, frame_buff,
                                            Ardcam.cam.totalLength);
                    }
                    break;
                }
            ...
            debug_printf("%s-%d", at_cmd[esp_cmd_idk], esp_cmd_idk);
        }

        if (cameraHeartBeat(&Ardcam.cam)) {
            switch (Ardcam.status) {
            case Camera_close:
            case Camera_init:
                begin(&Ardcam.cam);
                Ardcam.status = Camera_open;
                debug_printf("##############-start");
                break;
                // case Camera_open:
                //     while (buffer_ready != false)
                //         tight_loop_contents();
                //     camera_get_picture(&Ardcam.cam);
                //     camera_idx = idx;
                //     buffer_ready = true;
                //     break;
            }
        } else {
            Ardcam.status = Camera_close;
        }
    }
    return 0;
}