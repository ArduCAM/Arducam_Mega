...
void setup() {
    ...
    Xoff = 160; // positions the center of the 3d conversion space into the
                // center of the OLED screen. This is usally screen_x_size / 2.
    Yoff = 120; // screen_y_size /2
...
void RenderImage(void) {
    ...
    if (oarrow_x != 160 && 120 != oarrow_y)
        draw_fill_arrow(160, 120, oarrow_x, oarrow_y, 10, WHITE);
    ...
    if (arrow_x != 160 && 120 != arrow_y)
        draw_fill_arrow(160, 120, arrow_x, arrow_y, 10, GREEN);
    ...
    display_text_color(10, 120, display_buff, BLACK,12);
    sprintf(display_buff, "AcY: %.2f", AcY);
    display_text_color(10, 135, display_buff, BLACK,12);
    sprintf(display_buff, "AcZ: %.2f", AcZ);
    display_text_color(10, 150, display_buff, BLACK,12);
...
void loop() {
    ...
    for (int i = 0; i < LinestoRender; i++) {
        ORender[i] =
            Render[i]; // stores the old line segment so we can delete it later.
    ...
    oarrow_y = arrow_y;
    arrow_x =
        (Render[0].p0.x + Render[1].p0.x + Render[2].p0.x + Render[3].p0.x) / 4;
    arrow_y =
        (Render[0].p0.y + Render[1].p0.y + Render[2].p0.y + Render[3].p0.y) / 4;
