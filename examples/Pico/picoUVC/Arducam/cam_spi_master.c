...
{
	uint8_t rxDat;
	spi_write_read_blocking(CAMERA_SPI_PORT, &value, &rxDat, 1);
	return rxDat;
}

int spi_write_block(uint8_t *p_value, size_t len)
{
	return spi_write_blocking(CAMERA_SPI_PORT, p_value, len);
}

int spi_read_block(uint8_t *ptr_value, size_t len)
{
	return spi_read_blocking(CAMERA_SPI_PORT,0x00, ptr_value, len);
}

void spi_begin()
{
	spi_init(CAMERA_SPI_PORT, 12*1000*1000);
